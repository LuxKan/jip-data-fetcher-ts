...
const POOL_PUBKEY = new PublicKey("5BUwFW4nRbftYTDMbgxykoFWqWHPzahFSNAaaaJtVKsq");
...
async function main() {
  ...
  // Print each asset individually with icon and delta
  for (const symbol of Object.keys(custodyData)) {
    printAssetData(
      symbol,
      custodyData[symbol].custody,
      custodyData[symbol].decimals,
      supply
    );
  }
}
...
function printAssetData(
  ...
  console.log(`Max Global Long Sizes       : ${Number(custody.pricing.maxGlobalLongSizes) / 10 ** 6}`);
  console.log(`Max Global Short Sizes      : ${Number(custody.pricing.maxGlobalShortSizes) / 10 ** 6}`);
  console.log(`Fees Reserves               : ${Number(custody.assets.feesReserves) / 10 ** decimals}`);
  const maxLeverage = Number(custody.pricing.maxLeverage) / 10 ** 6;
  ...
  const feeReserves = Number(custody.assets.feesReserves) / 10 ** decimals;
    ...
    console.log(`Guaranteed USD              : ${Number(custody.assets.guaranteedUsd) / 10 ** 6}` );
    console.log(`Global Short Sizes          : ${globalShortSizes}`);
    console.log(`Global Short Average Prices : ${Number(custody.assets.globalShortAveragePrices) / 10 ** 6}` );

  // Delta calculation
  const delta = (owned + globalShortSizes + feeReserves) / jlpSupply;
    console.log(`Exposure                    : ${delta}`);
